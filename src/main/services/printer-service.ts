...
import { PURCHASE_PAYMENT_STATUS_OPTIONS } from "@shared/types/purchase";
  ...
  for (const charge of serviceCharges) {
    ...
    rows.push(`
      <tr>
        ...
        <td class="right">-</td>
        <td class="right">${money(charge.lineTotalCents)}</td>
      </tr>`);
  }
  ...
  <div class="sheet">
    ...
    <div class="items-frame">
      ...
      <table class="totals-table">
        ...
        ${sale.includeTaxBreakdown && computeAddedTaxCents(withTaxableServiceCharges(sale.items, sale.serviceCharges)) > 0 ? `<tr><td>Total Tax</td><td>${money(computeAddedTaxCents(withTaxableServiceCharges(sale.items, sale.serviceCharges)))}</td></tr>` : ""}
    ...
    ${sale.includeTaxBreakdown ? buildTaxBreakdownHtml(computeTaxBreakdown(withTaxableServiceCharges(sale.items, sale.serviceCharges)), business.vatRatePercent, (cents) => money(cents), "tax-breakdown") : ""}
...
<body>
  <div class="receipt">
    ...
    <table class="totals">
      ...
      ${sale.includeTaxBreakdown && computeAddedTaxCents(withTaxableServiceCharges(sale.items, sale.serviceCharges)) > 0 ? `<tr><td class="label">Total Tax</td><td class="right">${money(computeAddedTaxCents(withTaxableServiceCharges(sale.items, sale.serviceCharges)))}</td></tr>` : ""}
    ...
    ${sale.includeTaxBreakdown ? buildTaxBreakdownHtml(computeTaxBreakdown(withTaxableServiceCharges(sale.items, sale.serviceCharges)), business.vatRatePercent, (cents) => money(cents), "items") : ""}
  ...
  <div class="sheet">
    ...
    <div class="items-frame">
      ...
      <table class="totals-table">
        ...
        ${quotation.includeTaxBreakdown && computeAddedTaxCents(withTaxableServiceCharges(quotation.items, quotation.serviceCharges)) > 0 ? `<tr><td>Total Tax</td><td>${money(computeAddedTaxCents(withTaxableServiceCharges(quotation.items, quotation.serviceCharges)))}</td></tr>` : ""}
        <tr class="grand"><td>Total</td><td>${money(quotation.grandTotalCents)}</td></tr>
      </table>
    </div>

    ${quotation.includeTaxBreakdown ? buildTaxBreakdownHtml(computeTaxBreakdown(withTaxableServiceCharges(quotation.items, quotation.serviceCharges)), business.vatRatePercent, (cents) => money(cents), "tax-breakdown") : ""}
...
<body>
  <div class="receipt">
    ...
    <table class="totals">
      ...
      ${quotation.includeTaxBreakdown && computeAddedTaxCents(withTaxableServiceCharges(quotation.items, quotation.serviceCharges)) > 0 ? `<tr><td class="label">Total Tax</td><td class="right">${money(computeAddedTaxCents(withTaxableServiceCharges(quotation.items, quotation.serviceCharges)))}</td></tr>` : ""}
      <tr class="grand"><td class="label">Total</td><td class="right">${money(quotation.grandTotalCents)}</td></tr>
    </table>
    ${quotation.includeTaxBreakdown ? buildTaxBreakdownHtml(computeTaxBreakdown(withTaxableServiceCharges(quotation.items, quotation.serviceCharges)), business.vatRatePercent, (cents) => money(cents), "items") : ""}
...
function purchasePaymentStatusLabel(status: SupplierStatementViewModel["purchases"][number]["paymentStatus"]): string {
  return PURCHASE_PAYMENT_STATUS_OPTIONS.find((option) => option.value === status)?.label ?? status;
}

/** Mirrors buildStatementHtml exactly — a Supplier Statement of Account, same layout/letterhead, just
 * "what WE owe" instead of "what a customer owes US". Not tied to one storefront (a supplier's
 * purchases can span several), same reasoning as the customer version. */
function buildSupplierStatementHtml(vm: SupplierStatementViewModel): string {
  const money = (cents: number): string => `${vm.currency} ${formatReceiptCents(cents)}`;
  ...
  const rows =
    vm.purchases
      ...
      <tr>
        ...
        <td>${formatInvoiceDate(purchase.orderedAt)}</td>
...
<body>
  <div class="sheet">
    ...
    <div class="bill-to">
      ...
      <p class="name">${escapeHtml(vm.supplierName)}</p>
      <p class="muted">${escapeHtml(vm.supplierPhone)}</p>
      ${vm.supplierEmail ? `<p class="muted">${escapeHtml(vm.supplierEmail)}</p>` : ""}
    </div>
    ...
    <div class="items-frame">
      ...
      <div class="items-spacer"></div>
      <table class="totals-table">
        <tr><td>Total Ordered</td><td>${money(vm.totalOrderedCents)}</td></tr>
        <tr><td>Total Paid</td><td>${money(vm.totalPaidCents)}</td></tr>
        <tr class="balance"><td>Total Outstanding</td><td>${money(vm.totalOutstandingCents)}</td></tr>
      </table>
    </div>

    <div class="footer">Generated by ${escapeHtml(vm.businessName)} — internal record of what's owed to this supplier.</div>
  </div>
</body>
</html>`;
}
...
export async function generateSupplierStatementPdf(supplierId: string, filters: unknown = {}): Promise<string | null> {
  ...
  const result = await dialog.showSaveDialog({
    title: "Save Statement",
    defaultPath: `Statement-${vm.supplierName.replace(/[^a-z0-9]+/gi, "-")}.pdf`,
    filters: [{ name: "PDF", extensions: ["pdf"] }]
  });
  if (result.canceled || !result.filePath) {
    return null;
...
export async function previewSupplierStatementPdf(supplierId: string, filters: unknown = {}): Promise<void> {
  ...
  await openPdfPreviewWindow(buffer, filename, `Statement — ${vm.supplierName}`);
}
...
export async function printSupplierStatementDocument(supplierId: string, filters: unknown = {}): Promise<PrinterActionResult> {
  ...
  try {
    ...
    return { success: false, message: err instanceof Error ? err.message : "Failed to print supplier statement" };
  }
}
