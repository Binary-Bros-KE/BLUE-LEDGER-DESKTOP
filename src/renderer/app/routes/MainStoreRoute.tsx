...
export function MainStoreRoute(): React.JSX.Element {
  ...
  async function handlePickImage(): Promise<void> {
    ...
    try {
      ...
      setCreateError(getErrorMessage(err, "Failed to attach image"));
    } finally {
      setImageBusy(false);
    }
  }

  async function submitCreateProduct(event: React.FormEvent): Promise<void> {
    ...
    try {
      await window.blueLedger.product.create({
        sku: form.sku,
      ...
      setCreateError(getErrorMessage(err, "Failed to create product"));
    } finally {
  ...
  return (
    ...
    >
      ...
      <Modal
        ...
        title="New Product"
        description="Choose which storefront this product belongs to, or leave it available to all of them."
      ...
      >
        <form onSubmit={submitCreateProduct}>
          ...
          <p className="mt-2 text-[11px] font-semibold text-muted">
            "All Storefronts" means every storefront can see and sell it. A specific storefront means only
            that one can.
          </p>
          ...
          {storefronts.length > 0 && (
            <div className="mt-5 border-t border-line pt-5">
              ...
              <p className="mt-1 text-[11px] font-semibold text-muted">
                Seeds a storefront's own shelf stock directly. To stock Main Store instead, use "Receive" from
                the product's Manage Stock screen after creating it.
              </p>
              <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
                {storefronts
